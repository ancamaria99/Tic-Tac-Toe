...
//positions for printing X sign in different squares from the board
...
int isMovesLeft(int board[DIM_BOARD][DIM_BOARD])
{
  for (int i = 0; i < DIM_BOARD; i++)
    for (int j = 0; j < DIM_BOARD; j++)
      if (board[i][j]==0)
...
int evaluate(int b[DIM_BOARD][DIM_BOARD])
{
...
int minimax(int board[DIM_BOARD][DIM_BOARD], int depth, int isMax)
{
...
Move findBestMove(int board[DIM_BOARD][DIM_BOARD])
{
  int bestVal = -1000;
  Move bestMove;
  bestMove.row = -1;
